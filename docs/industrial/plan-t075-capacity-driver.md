# T-075 容量证据负载驱动器 — 实施计划（目标正式门禁已通过）

状态：**2026-09-17 阶段 A–F 已完成，目标正式容量门禁通过。** 驱动器在 Ubuntu 22.04/RTX 5090 上完成真实 HTTP 负载、宿主资源采样、10 场景故障注入、直接 PostgreSQL 真实性查询、镜像 `tag@sha256` 固定和受限 loopback ENOSPC。最终报告覆盖 50,000 active chunks、20 用户、5 并发和 1,807.5 秒真实运行，状态为 `formal=true/status=passed`；证据下载验哈后目标实例已自动关机。历史 readiness 与两次失败归档继续保留，不冒充最终 PASS。
...
T-075 的负载驱动器、失败取证与断点续跑链路已经落地，最终目标报告通过全部正式门禁；
该结果证明当前受控配置在既定规模和场景下达标，不外推为生产 SLA。
...
| 阶段 | 内容 | 地点 | 成本 | 完成标志 |
|---|---|---|---|---|
| **A** | 驱动器骨架 + manifest 构建器 + 真实性守卫 | 本机 | ¥0 | **完成** |
| **B** | 语料生成器：精确 50000 块 + 20 用户（可复现种子） | 本机 | ¥0 | **完成**：20 × 2500 |
| **C** | HTTP 负载引擎（Cookie/CSRF/Origin、5 并发、幂等键、时延埋点） | 本机 | ¥0 | **完成** |
| **D** | 解析/重索引与查询并行、长时稳态观测 | 本机 | ¥0 | **完成** |
| **E** | 6 个故障场景 + 采样器 + 正式真实性门禁 | 本机 | ¥0 | **完成**：10/10 |
...
正式结果位于 `artifacts/t075-target-runs/t075-formal-resume-20260916-200000/`：归档 SHA-256 为
`db363bd8998957858e44db2378e7deb1bef3f73e95e1007d1ca683aa1d066ad2`，报告为
`formal=true/status=passed`，无失败项。首次无 Git 发布包取证失败和首轮显存配置基线失败均原样保留；
最终复跑没有修改阈值，也没有重建已经完成的 50,000-chunk 索引。

下一步不再重复烧卡。只有代码、模型配置或目标硬件发生实质变化时才重跑 T-075；当前主线转向真实用户
pilot 授权与 target-release 汇总。
