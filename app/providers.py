...
GENERATION_SYSTEM = (
    ...
    '使用最少数量的 requirements，只覆盖 question 明确要求的事实，通常一到四项；'
    '不得为题目未要求的其他机制、更多细节或外延信息创建缺口。比较题不得把对象与属性机械组合，'
    '只记录题目明确要求且 evidence 实际定义的差异。忽略与问题实体无关的 evidence，不得据此新增 requirement。'
