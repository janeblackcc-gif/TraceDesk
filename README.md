...
当前版本为 **0.2.0rc1**，首发目标是可复现的本地单人部署。后端使用FastAPI与SQLite，网页为原生HTML/CSS/JavaScript，本地模型通过Ollama调用。
...
新增24题FastAPI外部文档挑战已完成72次问答：三种方法Hit@4均为18/18，但同时要求答案完整、逐项引用支持和正确拒答状态时，均仅通过13/24。已发现把构建进度当成镜像大小等错误，详见[完整报告与失败案例](docs/external-evaluation.md)。当前继续保留RC1预发布状态。
